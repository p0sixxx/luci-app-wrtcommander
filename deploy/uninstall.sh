...
#
# --purge   also remove /etc/config/wrtcommander (kept by default in
...
echo "== Wrt Commander uninstaller =="
...
while IFS= read -r line; do
	...
	if [ "$dst" = "/etc/config/wrtcommander" ] && [ "$PURGE" != "1" ]; then
...
rm -f /tmp/wrtcommander-debug.log 2>/dev/null || true
...
echo
echo "Wrt Commander has been uninstalled."
if [ "$PURGE" != "1" ]; then
	echo "Configuration kept at /etc/config/wrtcommander - remove manually or re-run with --purge."
fi
